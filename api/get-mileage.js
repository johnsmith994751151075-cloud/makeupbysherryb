...
module.exports = async (req, res) => {
  ...
  if (result.status === 200) {
    ...
    return res.status(200).send(result.body.toString('utf8'));
  } else if (result.status === 409) {
    // File doesn't exist yet — return empty CSV with headers
    res.setHeader('Content-Type', 'text/plain');
    return res.status(200).send('Date,Client,Destination,Miles (Round Trip),IRS Deduction,Notes\n');
  } else {
    return res.status(500).json({ success: false, error: `Dropbox error: ${result.status}` });
  }
};
